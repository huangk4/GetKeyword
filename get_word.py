...
#数据加载，返回进行分词和过滤后的数据，pos为是否词性标注的参数，corpus_path为数据集路径
def load_data(pos=False,corpus_path='./corpus.txt',encoding='utf-8'):
    #调用上面方式为数据集处理，处理后的每条数据仅保留非干扰词
    doc_list=[]
...
#TF-IDF类
class TfIdf(object):
    #四个参数为：训练好的idf字典，默认idf值，处理后的待提取文本，关键词数量
...
class TopicModel(object):
    #三个参数为：处理后的数据集，关键词数量，具体模型(LSI、LDA)，主题数量
    def __init__(self,doc_list,keyword_num,model='LSI',num_topics=4):
        #使用genism的接口，将文本转为向量化表示
        #先构建词空间
    ...
    #计算词的分布和文档的分布相似度，取相似度最高的keyword_num个词作为关键词
